interface AboutCardProps {
  ...
  category: string;
  title: string;
  content: string | string[];  // content теперь может быть как строкой, так и массивом JSX элементов
}
...
export const AboutCard: React.FC<AboutCardProps> = ({ category, title, content }) => {
  ...
  return (
    <div className="rounded-[20px] overflow-hidden bg-white shadow-md hover:transform transition-transform duration-300 hover:scale-130 hover:shadow-lg">
      <div className={`flex items-center justify-center h-[120px] max-sm:h-[80px] bg-[linear-gradient(to_right,#6753FF,#5954AA)]`}>
        <h1 className="font-fustat font-extrabold text-[64px] max-sm:text-[40px] text-center text-white/30">{category}</h1>
      </div>
      <div className="p-5 h-[300px] max-sm:h-[340px] pb-[20px]">
        <h3 className="text-lg  font-fustat font-semibold mb-2">{title}</h3>
        {isSkillsCategory &&
        <div><p className={'mb-4 text-sm  max-sm:text-xs text-gray-500'}>I use an advanced technology stack to develop efficient and scalable solutions that enable the creation
          ...
          {Array.isArray(content) ? (
            content.map((item, index) => (
              <div
                ...
                key={index}
              >
                <span className="text-sm  max-sm:text-xs text-gray-500">{item}</span>
              </div>
            ))
          ) : (
